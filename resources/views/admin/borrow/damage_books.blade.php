                ...
                <div class="row align-items-center g-3">
                    <div class="col-12 col-md-4">
                        <h5 class="mb-0 fw-bold text-danger">
                            <i class="bi bi-x-circle me-2"></i> {{ __('Damage Books & Penalties') }}
                        </h5>
                    </div>
                    ...
                    <div class="col-12 col-md-8">
                        <div class="flex-wrap gap-2 d-flex align-items-center justify-content-md-end">
                            ...
                            <div class="px-2 py-2 text-white rounded shadow-sm bg-danger text-nowrap"
                                ...
                                {{ __('Total Damage Fine') }}: <span
                                    class="fw-bold">{{ number_format($damageBooks->sum('damage_fine')) }}
                                    {{ __('MMK') }}</span>
                            </div>
                            <form action="{{ route('admin.exportDamageBooks') }}" method="GET" class="m-0">
                            ...
                            {{-- Search --}}
                            <form action="{{ route('admin#damageBooksList') }}" method="GET" class="flex-grow-1"
                ...
                <table class="table mb-0 align-middle table-hover" style="min-width: 800px;">
                    <thead class="table-light text-nowrap">
                        <tr>
                            ...
                            <th>{{ __('Returned') }}</th>
                            <th>{{ __('Damage Fine Amount') }}</th>
                        </tr>
                    </thead>
                    <tbody class="text-nowrap">
                        @forelse($damageBooks as $req)
                            <tr>
                                <td class="ps-4">
                                ...
                                <td>
                                    <span
                                        class="badge bg-danger-subtle text-danger fs-6">{{ number_format($req->damage_fine) }}
                                        {{ __('MMK') }}</span>
                                </td>
                            </tr>
                        @empty
