...
class DamageBooksExport implements FromCollection, WithHeadings, WithMapping, WithCustomStartCell, WithEvents
{
    protected $search;
    ...
    public function headings(): array {
        return ['User Name', 'Roll Number', 'Year', 'Book Title', 'Book Code', 'Returned Date', 'Fine Amount'];
    }
    ...
    public function map($row): array {
        return [
            ...
            number_format($row->damage_fine) . ' MMK', // 👈 ဤနေရာတွင် တန်ဖိုးတစ်ခုတည်းကိုသာ MMK ဖြင့် ပေါင်းထုတ်ပေးရန်
        ];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'Damage Books Report');
                ...
                // Column F မှာ 'Total Damage Fine:' စာသား၊ Column G မှာ တန်ဖိုးကို ထည့်ခြင်း
                $sheet->setCellValue('F' . $totalRow, 'Total Damage Fine:');
                $sheet->setCellValue('G' . $totalRow, number_format($data->sum('damage_fine')) . ' MMK');
